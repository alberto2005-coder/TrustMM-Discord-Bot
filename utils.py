...
async def send_ltc(recipient_address, amount, interaction_channel, deal_id):
    ...
    private_key = deal_data.get("private_key")  
    sender_address = deal_data.get("ltc_address") 
    ...
    try:
        ...
        embed.add_field(name="To:", value=recipient_address, inline=True)
        embed.add_field(name="Amount Sent:", value=f"{amount_after_fee} LTC", inline=True)
...
async def complete_deal(deal_id, interaction):
    ...
    embed.add_field(name="Deal ID:", value=deal_id, inline=True)
    embed.add_field(name="Amount:", value=f"${amount_usd}", inline=True)
    ...
    mm_embed.add_field(name="Amount:", value=f"${amount_usd}", inline=True)
